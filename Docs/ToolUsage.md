...
<br>``-b`` Generates Line ID Code with a line ID
...
<br>``DoCTextTool.exe -g "MES_70352CT" ``
...
- The extracted text file will have a line Id code, a line Id and the line associated with that Id. each line in the text file will be neatly separated with a `` || `` symbol and will look something like this:
...
- If you are adding a new line, then ensure that both the 'line Id code' and your new 'line Id' are present before your line data, with the `` || `` symbol separators. use the ``-g`` switch to generate a 'line Id code' for your new 'line Id'. 
- The order of the lines in the text file, will depend on the 'line Id code' sequence. for example, you can't have ``1580148303`` after ``1781806664``.
